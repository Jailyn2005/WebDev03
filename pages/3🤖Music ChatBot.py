...
# ---- Demo Mode Toggle ----
demo_mode = st.checkbox("Run in Demo Mode (no Spotify connection)", value=False)
...
if demo_mode:
    ...
    st.success("🎶 Demo Mode: Loaded sample track *Levitating* by *Dua Lipa*")
else:
    song_data = get_track_features(song_query) if song_query else None

# ---- Song Data Display & Chat ----
if song_data:
    if song_data.get("error") == "403":
        st.warning("⚠️ Spotify blocked access to vibe data for this song (403 error). Try another track — some older songs are restricted.")
    elif song_data["valence"] is not None:
